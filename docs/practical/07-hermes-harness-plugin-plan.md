...
- No JESUSCORD-specific roles. growth, eng, brand, room-engine, video, distro, sermons, creators, dev, churches are *example* domain specialists, not part of the plugin.
...
Six profiles, all on the orchestration host, all reading and writing `factory/` per `06_protocol.md` from the team docs. Cycle cadence and behavior follow the boss/supervisor/hr/conductor specs in `03_top_tier_souls.md` exactly. Two additions: `critic` and `a2a-bridge`.

### 3.1 boss / supervisor / hr / conductor

Unchanged from `~/projects/hermes-harness/docs/team/03_top_tier_souls.md`. Domain-specific clauses are stripped (no growth/eng/brand references in the SOULs that ship; users add domain context in their own `AGENTS.md`).
...
The factory bus is markdown. Mirror it to an Obsidian vault per the existing pattern in `~/projects/hermes-harness/docs/team/00_OVERVIEW.md`:113.
...
- Boss's strategic memos at `wiki/lessons/strategic_<date>.md` (per `03_top_tier_souls.md` §boss "Every 4h").
...
- `~/projects/hermes-harness/docs/team/00_OVERVIEW.md` — architecture diagram + bus structure (the Obsidian-mirror pattern at line 113)
- `~/projects/hermes-harness/docs/team/03_top_tier_souls.md` — boss/supervisor/hr/conductor SOULs (strip domain specifics for the plugin's generic versions)
- `~/projects/hermes-harness/docs/team/06_protocol.md` — PROTOCOL.md (copy verbatim into bus_template/)
